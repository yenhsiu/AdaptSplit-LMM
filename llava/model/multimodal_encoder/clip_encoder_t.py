...
class CLIPVisionTower(nn.Module):
    ...
    def load_model(self):
        ...
        self.compressor = MSECompressor(head_dim=1024, bits=1, seed=42, device="cuda")
    ...
    def token_prune_merge_advanced(self, images, if_adaptive=True, reduction_ratio = 1/8):
        ...
        # ── Stage 2: Attention + outlier detection + top-k + gather ─────────
        ...
        # Store per-call breakdown in ms (accumulate for averaging)
        entry = {
            "t_vit_ms":        (t1 - t0) * 1000,
            "t_attn_topk_ms":  (t2 - t1) * 1000,
            "t_merge_loop_ms": (t3 - t2) * 1000,
            "left_tokens":     left_tokens,
            "reduction_ratio": reduction_ratio,
        }
        if not hasattr(self, "latency_log"):
            self.latency_log = []
    ...
    def token_prune_merge_advanced_plus(self, images, if_adaptive=True, reduction_ratio = 1/8):
        ...
        # ── Stage 2: Attention + outlier detection + top-k + spatial gather ──
        attn = (desired_layer_q @ desired_layer_k.transpose(-2, -1)) * C ** -0.5
        attn = F.softmax(attn, dim=-1)
        ...
        # Store per-call breakdown in ms
        entry = {
            "t_vit_ms":        (t1 - t0) * 1000,
            "t_attn_topk_ms":  (t2 - t1) * 1000,
            "t_merge_loop_ms": (t3 - t2) * 1000,
        ...
        if not hasattr(self, "latency_log_plus"):
            self.latency_log_plus = []
        self.latency_log_plus.append(entry)
